    ...
    secret.write_text(
        "\n".join(
            [
                ...
                "SHARED_DEV_DB_USER=synthetic_user",
                ...
                f"SHARED_DEV_MEDIA_SSH_CONFIG_PATH={ssh_config}",
...
def test_launcher_and_stop_scripts_do_not_include_shared_migration_or_seed_commands():
    ...
    stop_script = (REPO_ROOT / "scripts" / "stop-local-dev.ps1").read_text(encoding="utf-8").lower()

    assert "alembic upgrade head" not in launcher
    assert "seed_dev_auth_users" not in launcher
    assert "stop-process -id $tunnelpid" in stop_script
    assert "createdby" in stop_script
