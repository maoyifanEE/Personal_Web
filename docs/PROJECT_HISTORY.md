...
* 从最新 main 创建 Feature/visitor-message-prototype 分支。
* 在公开首页增加右下角浮动留言入口。
* 增加访客留言前端弹窗原型。
* 增加留言管理端静态占位页。
* 在 Hub 中增加留言管理入口。
* 明确真实留言提交需要后端、数据库、认证和管理员权限。
* 不实现 localStorage、sessionStorage 或 cookie 留言持久化。
* 不新增后端、数据库、API、认证或授权。

### 实际完成

* index.html 增加留言浮动工具和留言弹窗结构。
* styles.css 增加留言按钮、弹窗和移动端样式。
* script.js 增加留言弹窗打开、关闭、Esc、overlay 和原型校验逻辑。
* hub.html 增加留言管理入口卡片。
* 新增 apps/messages/index.html 管理端 UI 原型。
* 新增 apps/messages/messages.css 管理端样式。
* 新增 apps/messages/messages.js 管理端原型初始化日志。
* 更新 README 和 docs，记录未来后端和数据库需求。
...
* [ ] 空昵称和空留言校验有效。
* [ ] 填写昵称和留言后提交显示未保存原型提示。
* [ ] 留言没有保存到 localStorage、sessionStorage 或 cookie。
* [ ] Hub 显示留言管理入口。
* [ ] 留言管理页面可以打开。
* [ ] 留言管理页面明确标记为 UI 原型。
* [ ] 未新增后端、数据库、API、认证或授权。
