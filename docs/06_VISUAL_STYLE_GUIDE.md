...
* The final journey route should be generated by global approximating spline fitting and fairing.
* Guide anchors are not mandatory pass-through points; they only guide the route.
* Smoothness and visual polish may override exact hand-drawn fidelity.
* Adjacent journey areas should align both path endpoints and boundary tangents.
* Vertically offset journey areas should still connect smoothly at their shared boundary without a visible kink.
* Debug metrics should be used to evaluate endpoint gaps, tangent improvement, turn angles, curvature spikes, and raw-to-final deviation.
* The curve debug overlay and debug JSON export may be used to inspect raw points, guide anchors, global controls, final spline samples, and boundary diagnostics.
